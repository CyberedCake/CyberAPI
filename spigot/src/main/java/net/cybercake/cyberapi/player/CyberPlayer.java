...
import net.cybercake.cyberapi.player.ping.PingSettings;
import net.cybercake.cyberapi.player.ping.PingSettingsBuilder;
import net.cybercake.cyberapi.player.userhead.UserHeadSettings;
...
public class CyberPlayer {
    ...
    public String getUserHead(@Nullable UserHeadSettings settings) throws IOException {
        if(settings == null) settings = new UserHeadSettingsBuilder().buildEmpty();
    ...
    public String getUserHead(int imageScale, boolean helmet, Character character, String[] lines) throws IOException {
        return getUserHead(new UserHeadSettingsBuilder().imageScale(imageScale).showHelmet(helmet).character(character).lines(lines).build());
    }
    ...
    @Nullable public LuckPermsData getLuckPermsData() {
        ...
        if(!CyberAPI.getInstance().getLuckPermsSupport().equals(Settings.FeatureSupport.SUPPORTED)) return null;
    ...
    public class LuckPermsData {
        ...
        protected LuckPermsData(User user) {
            this.user = user;
            this.uuid = getUniqueID(); // obtained from parent class
        }
        ...
        /**
         * Gets the display name of the {@link CyberPlayer} by combining {@link LuckPermsData#getPrefix()}, {@link LuckPermsData#getLuckpermsUsername()}, and {@link LuckPermsData#getSuffix()}
    ...
    public class OnlineActions {
         ...
         * @param message the message to send
         * @apiNote requires Mini Message support
         */
         ...
         * <br> <br>
         * To create a {@link PingSettings} instance, you must create a new {@link PingSettingsBuilder} and modify the settings, and then use {@link PingSettingsBuilder#build()} to build the contents.
        ...
        public String getColoredPing(@Nullable PingSettings settings) {
            if(settings == null) settings = new PingSettingsBuilder().build();
            String coloredPing = getColorFromPing(player.getPing(), settings) + String.valueOf(player.getPing()) + (settings.shouldShowMs() ? "ms" : "");
            return (player.getPing() <= settings.getLoading() ? ChatColor.DARK_GRAY + "Loading..." : coloredPing);
        }
        ...
        public ChatColor getColorFromPing(int ping, @Nullable PingSettings settings) {
            ...
            if(ping <= settings.getLoading()) return ChatColor.DARK_GRAY;
            else if(NumberUtils.isBetweenEquals(ping, settings.getGreenMin(), settings.getGreenMax())) return ChatColor.GREEN;
        ...
        public void printUserHead() throws IOException {
            printUserHead(new UserHeadSettingsBuilder().buildEmpty());
        }
    }
