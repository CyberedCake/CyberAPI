...
public class CommandManager {
    ...
    public void init(String path) {
        try {
            ...
            if(path == null) {
                Method method = CyberAPI.class.getDeclaredMethod("startCyberAPI", FinalizedSettings.class);
                CyberAPI.getInstance().getAPILogger().warn("Please specify a commands path/package to speed up command registering in " + method.getDeclaringClass().getCanonicalName() + "." + method.getName() + "(" + FinalizedSettings.class.getCanonicalName() + ")! (registering took " + (System.currentTimeMillis()-mss) + "ms!)");
            }
        } catch (Exception exception) {
